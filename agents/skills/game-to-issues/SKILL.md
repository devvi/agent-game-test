...
### 9. 画面风格（Visual Identity）— 必须决策，不能默认

> **用户必须给出画面风格描述。** 如果用户没有提供，agent 必须根据游戏的内容和机制自行推导画面风格，
> 并在分解时将画面风格拆解为具体的视觉 Issue（场景渲染、UI 风格、特效方案）。
>
...
Grill Me 阶段必须问（如果用户没主动提供）：

| 问题 | 目的 |
|------|------|
| 这个游戏看起来像什么？ | 不是"好看就行"，是具体的参考：极乐迪斯科的油画风、Into the Breach 的像素风、Journey 的极简风 |
| 画面风格服务于什么情绪？ | 孤独？紧张？荒诞？温暖？风格是情绪的视觉化 |
| 玩家第一眼看到的是什么？ | 标题画面、游戏内首帧——决定了玩家是否愿意继续 |
| UI 风格和游戏世界一致吗？ | UI 是游戏世界的一部分还是悬浮在外的 HUD？ |

#### 9.2 如果用户不提供画面描述

Agent 必须基于以下维度自行推导，并在分解中包含画面风格相关的 Issues：

| 维度 | 推导方式 | 示例 |
|:-----|:---------|:-----|
| **游戏类型** | CRPG → 文字/对话为重心 → UI 清晰可读，场景服务于氛围 | 雨夜普罗摩茨：Decal/GPUParticles 霓虹+雨 |
| **主题情绪** | 孤独/悬疑 → 暗色调、低饱和度、阴影对比 | Hollow Knight 的压抑感 |
| **核心机制** | 平台跳跃 → 色彩区分可交互/不可交互元素 | Celeste 的紫色调+白色可攀爬 |
| **目标平台** | 移动端 → 高对比度、大字、简化粒子 | 纪念碑谷的极简几何 |
...
### Step 0: Grill Me — 审美坐标诘问（个人化需求拷问）

> 在分解之前，先拷问用户到底想做什么。**但这不是"确认游戏类型"——是确认"什么样的游戏对你来说是好游戏"。**
> 通用分解产出"合格游戏"；审美坐标注入的分解产出**你的游戏**。
> 目标：产出一份**审美宣言 + 完成定义（Aesthetic Manifesto + Definition of Done）**，注入每一个后续 Issue。

**重要：这一步是交互式的。** 输出问题给用户，等待用户回答，再基于回答提出更深的问题。至少进行 3-4 轮问答（含审美坐标轮），直到你不仅能说清"做什么"，还能说清"它为什么是好的"。

#### 0.0 第零轮：审美坐标（Aesthetic Coordinates）— 挖出你的品味

> **为什么先问这个：** 引擎、平台、类型都是可替换的参数；**品味才是不可替换的内核**。
> 极乐迪斯科之所以特别，不是因为它"是个 CRPG"，而是因为它是一群有明确审美坐标的人做的。
> 这一轮产出的坐标将作为 Step 2 分解 prompt 的**最顶层上下文**——每个 Issue 的 context 和验收条件都必须回应它。

**必问的四个问题（如果用户没主动提供）：**

| 问题 | 目的 | 合格回答的样子 |
|------|------|---------------|
| **参考坐标系**：哪 3 个作品（游戏/电影/小说/音乐）是你心里"好"的样子？它们共同的氛围、手法、气质是什么？ | 建立品味的锚点 | 不是列名字，是说出共性："《极乐迪斯科》的失败即叙事 + 《银翼杀手》的雨夜氛围 + 海明威的短句克制" |
| **情感目标**：玩家玩完（哪怕是 30 分钟的 MVP）离开时，你希望他心里留下什么？ | 定义体验的靶心 | 不是"讲了个故事"，是"感到孤独但被理解""觉得自己的选择有重量" |
| **词汇表**：用 5 个形容词描述你想要的画面；用 3 个词描述文字/对白的质感 | 给 implement agent 可执行的风格指令 | "暗、湿、暖、霓虹、疏离" + "短句、冷、克制" |
| **反例（负空间）**：什么作品你觉得"无聊/做作/廉价"？为什么？ | 反向定义品味边界 | "为了文艺而文艺的独白" "亮度过高的卡通 UI" —— 反例比正例更能防止跑偏 |

**经验注入（第零轮的隐藏环节）：** 在提问之前/同时，先做两件侦查：
...
把命中结果作为"你过去的自己会怎么想"注入诘问，例如：
- "你的 GDD 里雨夜氛围那章用了'冷暖对比'，这个新游戏延续还是反着来？"
- "你 mini-pong 的霓虹赛博验证了'高饱和+暗底'可行，这个游戏用同一套视觉语言吗？"

**关键规则：** 用户给不出参考时，不要替用户发明——给出 2-3 个候选方向让用户选或否掉。用户说"不知道"本身是有效信息（说明品味未被调用），继续追问直到出现具体锚点。
...
#### 0.3 第三轮：完成定义 — 什么算"做完"（Definition of Done）

> **为什么必须有这一轮：** 分解的终极目标是"通过这些 Issues 能完成一个游戏"——不是"完成一堆功能"。
> 一个游戏做完的标志是：**玩家能从头到尾体验完整路径，且这段体验符合你第零轮的审美坐标。**
> 没有完成定义的 issue 集，最后会得到一堆"功能都做了但游戏不存在"的零件（参见 game-infrastructure-checklist 的"冰箱里的食物"）。

必须让用户明确四个层次：

1. **完成路径（Complete Path）** — 玩家从打开游戏到看到结局画面，具体经过什么？
   - 不要回答"做完核心功能"——具体到"玩家从 A 走到 B，和 C 对话，做出选择，看到 D 结束画面"
   - 这条路径上的每一步，都要有对应的 Issue 覆盖（缺一步 = 游戏不可完成）

2. **个人完成标准（Personal Definition of Done）** — 你（开发者）怎么判断"这游戏做完了"？
   - 不是"功能清单全勾"——是"当我看到/玩到 X 时，我知道它成了"
   - 例："当雨夜的第一分钟让我自己起鸡皮疙瘩" "当陌生人的对白让我想重玩一遍看不同选择"
   - 这个标准会成为最终验收 Issue（Integration/Playtest）的验收条件

3. **MVP 边界** — 什么功能可以被推迟到 v1/v2？
   - 主动建议推迟非核心功能（音效简化、附加结局）
   - MVP 的体验时长是多少？5 分钟？15 分钟？30 分钟？
   - **MVP 必须包含完整路径的每一层**（基础设施→逻辑→内容→表现→集成），可以浅，不能缺层
...
```json
{
  "clarified_brief": {
    ...
    "aesthetic_coordinates": {
      "reference_works": ["3 个参考作品及它们的共性"],
      "target_emotion": "玩家离开时心里留下的东西",
      "visual_vocabulary": ["5 个画面形容词"],
      "text_vocabulary": ["3 个文字质感词"],
...
这个 `clarified_brief` 将作为 Step 2 语义分解的输入上下文。**aesthetic_coordinates 和 definition_of_done 是分解 prompt 的最顶层指令**——每个 Issue 的 context 必须引用对应坐标，每个内容类 Issue 的验收条件必须包含审美合规检查。
...
**分解 prompt 必须包含（按此优先级注入）：**

```
1. 审美坐标（clarified_brief.aesthetic_coordinates）— 最顶层指令
   每个 Issue 的 context 必须引用对应坐标；每个内容类 Issue 的
   验收条件必须包含审美合规检查（"画面符合 visual_vocabulary"、
   "对白符合 text_vocabulary"、反例不得出现）
2. 完成定义（clarified_brief.definition_of_done）— 结构约束
   完成路径的每一步必须有 Issue 覆盖；最终必须有 Integration/Playtest
   Issue 承载 personal_standard 验收
3. 完整的用户命令原文 + Grill Me 输出（clarified brief）
4. 所有分解规则（粒度/依赖/优先级/深度/引擎专项/画面风格）
...
### Step 4.5: Completeness Gate — 强制完整性校验（必做，不可跳过）

> **为什么强制：** 分解的终极目标是通过这些 Issues **完成一个游戏**，不是完成一堆功能。
...
- [ ] 基础设施层（脚手架/CI）、数据层（存档/资源）、表现层（渲染/UI）、场景层、
      内容层（对话/叙事/音效）、逻辑层（状态/规则）、交互层（输入）、玩家体验层（3C/反馈）
      每一层都有 Issue 覆盖（可以浅，不能缺层）
- [ ] **交互层最常被跳过** — 明确检查"输入→可交互物体"的通信路径有 Issue

**C3. 胶水代码/集成** — 组件之间的连接有独立 Issue？
- [ ] 每个"跨组件信号/事件/回调"有 [Integration] Issue（不是假设"组件会自己连好"）
- [ ] 示例："对话结束时解锁玩家移动"、"选择后把 scene 发给 SceneManager"

**C4. 验收条件可测** — 每个 Issue 的 acceptance_criteria 可验证？
- [ ] 不是"感觉良好"式模糊标准（"画面好看" ❌）
