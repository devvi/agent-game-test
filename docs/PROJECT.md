...
| 681 | Shandong Wolf 攻击动画查询委托补漏 (StickFigureController 补 get_animation_position()/is_animation_playing() 两个 null-guard 薄委托（委托 _anim.current_animation_position / _anim.is_playing()，默认 0.0/false），一处实现同时修复 AnimStateAttack 主路径与 E2E capture 路径的缺失方法 SCRIPT ERROR，attack 三段 phase 判定恢复, #692) | ✅ 已合并 | DESIGN / GDD |+ execute_kill 杀敌通道（绕过 take_damage no-op 红线 + _is_final_dead 停摆）+ set_invincible 玩家无敌 + recover_from_break 起身疲惫（50% 架势 + 5s ×1.2 增伤）+ ExecutionFade 墙钟淡出 + 「处决演出」7 常量 # DRAFT 归 #584 + E2E execution 组 2 shot + 六组测试套件, AC1-AC5 全过, 慢动作/淡出/范围等 DRAFT 值待用户 AC5 裁决, #660) | ✅ 已合并 | DESIGN / GDD |
## 已知问题
