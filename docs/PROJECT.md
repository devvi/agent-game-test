...
| Shandong Wolf — EnemyAI 行为层 | `shandong-wolf/gdscripts/enemy_ai.gd`（CharacterBody2D 行为 FSM 持有：patrol/chase/attack/retreat + 120° 视线 6m 几何感知（无 raycast/Area2D）+ 决策门控（实体非 idle/move 不决策）+ 弹反抑制窗 0.5s + decide/_physics_process 分离 + @export elite_mode 精英档位 + hit_landed 订阅受击击退（#682）） | ✅ | GDD |
