...
| Shandong Wolf — 反馈 E2E 截图 rig | `shandong-wolf/gdscripts/e2e_feedback_capture.gd` + `scenes/e2e_feedback_capture.tscn`（CaptureRig 模式：current_state 轮询 + inject_feedback + digit 键 4/6/7/2 + 冻结效果帧）+ `e2e_shots.json` feedback group（3 shots） | ✅ | GDD |
...
| Shandong Wolf — 组装 E2E 截图 rig | `shandong-wolf/gdscripts/e2e_main_assembly_capture.gd` + `scenes/e2e_main_assembly_capture.tscn`（CaptureRig 4 态驱动）+ `e2e_shots.json` assembly 组（01_spawn_combat/02_parry_execute/03_fail_subtitle/04_afterglow 共 4 shot） | ✅ | GDD |（六组用例 A-F：触发全链路/触发边界/无敌交互/疲惫数值闭环/淡出清理/失败路径防回归，mock ReactionController 断言 trigger_feedback，run_tests.gd 挂载） | ✅ | GDD |
