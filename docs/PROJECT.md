...
| 编译 | ✅ 通过 |
| 可运行 | ✅ 能启动 |
| 可玩 | ✅ PONG://NEON（原 Mini Pong）: 完整状态机(MENU→SERVING→PLAYING⇌PAUSED→SCORED→GAME_OVER)、UI系统、球物理、挡板移动、AI对手、暂停(Escape)、音效合成、L0动态雨幕(GPUParticles2D)、失败屏(win/fail双分支+run数据+软冻结)、球速 HUD 实时显示；🟡 shandong-wolf（当前活跃游戏）: 逻辑地基就绪（WolfConstants 数值集中地 + StateMachineBase 状态机基类 + Game autoload）+ 战斗数值 DRAFT 集中表（#584 只狼基准 14 参数候补值）+ DebugCanvas F1 调参面板（#609，仅 debug build）+ 输入层就绪（Input Map 9 动作 + InputController 意图事件/时间戳缓冲 + PlayerController 加速度移动，#611）+ 战斗实体与状态机就绪（CombatEntity 数据容器/两段血/输入桥 + 11 态战斗状态机 + 战斗时序 DRAFT 分区，#618）+ stagger 态架势崩解修复（状态表 stagger/parry_success 补 stance_break + take_stance_damage execute 守卫，消除 illegal transition stagger -> stance_break 警告，#718/#727）+ 火柴人剪影骨架与关键帧动画（Line2D 程序化骨架 7 pivot + 11 态关键帧 + consume_state 契约 + additive 刀光，#612）+ 血条与架势条极简 HUD（两段式血条/双架势条/击杀与处决提示，程序化绘制零贴图，草稿已合并待用户 E2E 截图定稿，#627）+ 拼刀/弹反/架势崩解判定层（CombatJudge 逻辑帧窗口判定 + AttackWindow 窗口契约 + 弹反/拼刀/格挡/受击五结果事件 + 6 判定常量 # DRAFT，#626）+ 雪夜氛围层（#582/#613 四层系统已落地 main：三层视差雪幕 60/60/80 粒子 + 单 CanvasModulate 冷月光 #6e7684 + 水墨晕染 + 血色 vignette 契约；#624/#629 单 moon 层契约：唯一 moon 挂 layer 0 + 雪幕/水墨/血色/UI 禁染 + C3 ==1 守卫；NIGHT_BG_COLOR 等 # DRAFT 定稿归 #582 用户裁决）+ 两条命原地复活系统（ReviveOrchestrator 自动复活编排：died(final=false)→1s 计时→revive()，与 F 键手动路径双路径幂等收敛 + ReviveFX 演出四件套：墨点 burst/瞬态闪屏/慢动作/无敌闪烁，12 演出常量 # DRAFT 归 #584，SW-015 终态契约固化，#637） + 基础日本兵 AI（EnemyAI 行为状态机：巡逻/追击/攻击/被弹反 4 行为态 + 120° 视线 6m 几何感知（无 raycast/Area2D）+ 决策门控 + 弹反抑制窗 0.5s；判定层 3 处 additive 参数化 + AI 分区 18 常量 # DRAFT，AC1-AC5 全过，#638）+ 雪夜山东村战斗舞台（battle_stage.tscn 纯声明式世界层场景：单一连续碰撞面 2400px + 视觉 3 段雪堆高差 60-100px + 草屋×2/枯树×2 墨色剪影 + 山峦远景 + 苍月 Mesh2D 径向渐变 + moon_glow 光晕 shader + PlayerSpawn/EnemySpawnA/B 出生点 + StageCamera limits 2400，零贴图零新增 CanvasModulate（C3 守卫），E2E battle_stage 组截图待用户 AC5 裁决，#646）+ 打击反馈系统（ReactionController 组合触发核心：FEEDBACK_MATRIX 9 事件×6 维分级矩阵（S/A/A-/B/C 六级 + clash/revive/death 补充映射）+ 五效果组件：苍白金 one_shot 火花（#ffd9a0，z_index -1 不盖角色；#675 背景 z=-2 显式化：背景<火花<角色，E2E 截图恢复火花）/ TimeScaleStack 时间缩放栈（D1 min 语义 + 墙钟兜底 + 栈深 3）/ ScreenShake trauma² 屏震（camera_path 解耦）/ FlashEffect 实体+全屏白闪双通道（全屏淡闪仅 A- 级）/ S 级复用 SwordArc 刀光；bind_judge 直连 #577 五结果事件 + subscribe_entity 订阅 #575 6 信号（闭包捕获实体 D2/D4）+ 降级路径；E2E 同帧截图 rig 三档 shot（fb_parry_success/fb_stance_break/fb_execute）冻结效果帧模式供用户 AC6 裁决，28 用例测试套件，反馈分区 12 常量 # DRAFT 归 #584，#654） + 处决系统（ExecutionOrchestrator 处决编排：stance_broken→armed 窗口 3s → 攻击键+距离≤120px 触发处决时序（玩家无敌→execute 转移→execute_kill 杀敌→S 级反馈→淡出）；execute_kill 专用杀敌通道绕过 take_damage execute no-op 无敌红线 + _is_final_dead 停摆守卫；ExecutionFade 墙钟驱动淡出 0.3s 如墨迹消散（慢动作不卡淡出）；recover_from_break 起身疲惫：恢复 50% 架势 + 5s 疲惫期受架势伤害 ×1.2，乘数下沉实体层；「处决演出」分区 7 常量 # DRAFT 归 #584（慢动作三候选并列禁止偷定）；六组测试套件；E2E execution 组 2 shot 待用户 AC5 裁决，#660） + MVP 战斗闭环组装（MainBattle BattleAssembler：13 步程序化装配接线 + 游戏状态机 IDLE→COMBAT→KILL→AFTERGLOW→FAIL（FAIL 终态幂等） + 失败字幕（输入冻结 + AI 停止 + 淡入） + 击杀 5s 余韵 + 教学提示 + 低血氛围单点接线 + Main.tscn 追加挂载 + smoke AC4 闭环自动跑通 —— 组装完成即游戏可玩，#666）+ E2E 剧本验收（e2e_script 组 6 帧情感弧剧本：scene_description/trigger/composition 文字契约 AC1；rig 4→7 态（MOVE/CLASH/EXECUTE + CYCLE_DWELL_FRAMES 定长停留 + move_displacement_px 位移断言）；管线四修复：组级场景键提升/数字 state 直比/分辨率从 project.godot 读取/--size 尺寸断言；headless 三档语义；docs/TEST/586 报告模板，6 帧 1-5 星裁决归用户（AC4），#673）+ 攻击查询委托 API（StickFigureController 补 get_animation_position()/is_animation_playing() 两个 null-guard 薄委托，修复 AnimStateAttack 与 E2E capture 缺失方法 SCRIPT ERROR，attack 三段 phase 判定恢复，#692）+ 精英 Boss AI（EnemyAI.elite_mode 参数档位：蓄力重斩三选一 + 受击击退位移 + 敌人架势脱战恢复 + EnemyHealthBar 敌人血条（与 EnemyStanceBar 双条组合）+ HP 慢线装配消费 ENEMY_HP_MAX=80，7 刀慢线击杀 / 4 弹反快线处决，#695）+ E2E 截图链路修复（resolve_plan 组级 autoplay 提升 + scene_groups 多场景拆分（run-e2e-review 按场景多次 capture 产物合并）+ e2e_shots.json feedback 组组级 autoplay 冻结接线 freeze_effects=true/auto_cycle_frames=200（> 最大 settle 120），fb 三档截图必落特效帧，战斗代码零改动，#661）+ 火柴人结构完整化（NeckPivot 颈段 + 头径 16→9.5 比例校准 + 可选 HeadOutline 冷白轮廓（实验 1）+ 腿两段化 LegKPivot 膝 pivot + anim_move 重排 24 帧步态循环（FRAME_ANIM_MOVE_CYCLE=24，contact/pass 关键姿态 + 膝 track + 摆臂反向同频）+ REST_POSE 公共基准衔接规约（动作型首尾帧 = REST_POSE / hold 型归位段 FRAME_ANIM_*_EXIT=3，姿态差 ≤ POSE_DELTA_MAX_DEG=15°）+ set_move_speed 步频速度同步（speed_scale 0.3–1.2）+ facing 视觉翻转接线（_sync_visual_facing → StickFigure.scale.x，物理层零影响），AC1-AC3 全过，taste 实验 1/2/3 待用户 E2E 裁决，#694） + 敌人 Boss 血条 UI（只狼式顶部血条+架势条组合：#684 呈现层增量——EnemyNameLabel 敌人名字 Label + EnemyStanceBar 崩解闪白状态机 + set_boss_mode Boss/杂兵分档三态显隐 + 装配 Boss 档「雪夜刀客」占位文案，5 新常量 # DRAFT 归 #584，#701） + 腿 Line2D 方向修正（#704 bug：腿/膝几何 -Y → +Y —— _reverse_limb 辅助函数翻转 Line2D points 方向（长度不变）+ 膝 pivot 移至 (0,+UPPER) + controller 摆姿注释 ±Y 勘误，腿自髋自然下垂不再重叠躯干；动画 11 clip track 数值零改动红线，D1 方向断言 6 条回归，AC1-AC3 全过，#709） + 敌人 AI 运行时驱动链（EnemyAI._physics_process 先 decide 后 _apply_movement：decide 纯决策回归（移除内部位移调用，杜绝双重位移）+ 位移每帧恰一次 + 击退无条件可达（stagger 硬直中击退照常执行，AC3）+ 测试 _tick 迁移运行时路径（TC703-1~4，36→48 用例），修复「敌人站桩」bug（#695 E2E 漏网根因），AC1-AC5 全过，#710）+ 敌人出生点外移（battle_stage.tscn EnemySpawnA 出生点 (700,560)→(1000,560)：出生间距 360px > HITBOX_RANGE=80px 消除开局空气命中，修布局不修判定——constants/combat_judge/enemy_ai/main_battle 零改动冻结清单 + test_battle_stage 新增 B4 出生间距断言（> HITBOX_RANGE+50 硬门槛 ≥130px 防回归），AC1-AC3 全过，#716） + ESC 暂停菜单与操作手册（PauseMenu 树级暂停 get_tree().paused + 自检 game_pause 边沿（A2：InputController 零改动）+ 操作手册运行时从 InputMap 生成（AC3 机器保证）+ FAIL 终态守卫，菜单标题/手册措辞/遮罩色值 # DRAFT 归 taste 通道，#719/#728） + Boss级 AI 战斗交互升级（敌人蓄力期霸体不被打断 + 玩家攻击自动面向 + 停距/击退缩短（ENEMY_ATTACK_RANGE 65 / ENEMY_KNOCKBACK_PX 22）+ 防御行为 GuardState 格挡 + 差异化前摇（combo 12/thrust 16/charge 24）+ 弹反窗口/回报差异化（0.2s/35、0.25s/45、0.3s/55），#720/#730） |
| 最近构建 | 2026-08-21（shandong-wolf Boss级 AI 战斗交互升级 #720/#730（敌人蓄力期霸体 + 玩家攻击自动面向 + 停距/击退缩短 + 防御行为 + 差异化前摇/弹反窗口回报）；此前 ESC 暂停菜单与操作手册 #719/#728（PauseMenu 树级暂停 + InputMap 生成操作手册）；此前 stagger 态架势崩解修复 #718/#727：状态表 stagger/parry_success 行补 stance_break + take_stance_damage execute 守卫，消除 illegal transition stagger -> stance_break 警告；此前 敌人出生点外移 #716 / 敌人 AI 运行时驱动链 #710 / 腿 Line2D 方向修正 #709 / 敌人 Boss 血条 UI #701 / 火柴人结构完整化 #694 / 精英 Boss AI #695 / E2E 截图链路修复 #678） |
| 开放 Issues | 1 |
...
| Shandong Wolf — CombatEntity | `shandong-wolf/gdscripts/combat_entity.gd`（Node2D 战斗数据容器：两段血/架势/facing + request_transition 唯一转移入口 + 6 信号 + _StateInputBridge 输入桥 + 蓄力重斩瞬时 override ×2（current_windup_frames/current_hp_damage）+ 敌人架势脱战恢复（#682）+ take_stance_damage execute 守卫（#718）+ 敌人蓄力期霸体（_is_armored windup 分支，弹反路径绕过）+ 玩家攻击自动面向（_face_nearest_target/_auto_face_target，#720）） | ✅ | GDD |
| Shandong Wolf — CombatStateTable | `shandong-wolf/gdscripts/combat_state_table.gd`（11 态转移合法性表 CANONICAL_STATES/TRANSITIONS/is_legal；stagger/parry_success 行补 stance_break，#718） | ✅ | GDD |
| Shandong Wolf — CombatStates | `shandong-wolf/gdscripts/combat_states.gd`（CombatStateBase + 11 状态对象 + make_state 工厂，帧/秒常量自动退出） | ✅ | GDD |
| Shandong Wolf — Combat Test Suite | `shandong-wolf/tests/test_combat_entity.gd`（30 用例：121 对转移遍历/受击/架势/两段血复活流/输入桥 mock；#718 stagger 中崩解/parry_success 崩解/execute no-op 用例 _test_d6/d7/d8） | ✅ | GDD |
| Shandong Wolf — StickFigure | `shandong-wolf/gdscripts/stick_figure.gd`（Node2D 程序化骨架：10 pivot（颈/膝两段化 #683）+ Line2D 肢体 + Polygon2D 头圆（BODY_HEAD_RADIUS 9.5）+ 可选 HeadOutline 冷白环 + sprite_slot 原画预留；scale.x = facing 视觉翻转目标（#694）+ 腿/膝 Line2D 向 +Y 方向（#704 _reverse_limb 方向翻转，长度不变，膝 pivot (0,+UPPER)）） | ✅ | GDD |
| Shandong Wolf — 动画控制器 | `shandong-wolf/gdscripts/stick_figure_controller.gd`（consume_state 契约 + 11 态→clip 镜像映射 + 过渡 ≤2 帧 + 动画资源运行时动态生成 + 查询委托 API：get_animation_position/is_animation_playing null-guard 薄委托，供 AnimStateAttack 与 E2E capture 查询，#681/#692；#683/#694 步态循环重排 24 帧 + REST_POSE 衔接规约 + set_move_speed 步频同步 + 摆姿注释 ±Y 勘误（#704：legs rotation=0 即自然下垂）） | ✅ | GDD |
...
| Shandong Wolf — Hud 极简 HUD | `shandong-wolf/gdscripts/hud.gd`（CanvasLayer layer=1：两段式血条/玩家与敌人架势条/击杀与处决提示/EnemyHealthBar 敌人血条（#682 顶部中央暗红条，与 EnemyStanceBar 双条组合，_HudBar.set_fill_color 覆写），纯信号驱动零轮询 + 内部类 _HudBar _draw 自绘 + low_health_changed 边沿信号 + EnemyNameLabel 名字 Label/set_break_flash 崩解闪白/set_boss_mode 分档三态显隐（#684）） | ✅ | GDD |
| Shandong Wolf — HUD E2E 截图 rig | `shandong-wolf/gdscripts/e2e_hud_capture.gd` + `scenes/e2e_hud_capture.tscn`（CaptureRig 模式 7 态：NORMAL/LOW_HP/EXECUTE_HINT/KILL_HINT + BOSS_BAR/STANCE_BREAK_FLASH/MINION_MODE，走 Hud 公有 debug API 零战斗场景依赖）+ `e2e_shots.json` hud group（7 shots） | ✅ | GDD |
| Shandong Wolf — HUD 测试套件 | `shandong-wolf/tests/test_hud.gd`（T1-T28 + #684 场景 A-F：名字布局/闪白状态机/分档三态/信号回归/E2E 态/装配断言；布局锚点/两段结构/低血边沿/敌人条显隐/提示竞争/回生/静态断言/单例守卫/数值防御，run_tests.gd 第 8 套件） | ✅ | GDD |
| Shandong Wolf — CombatJudge 判定器 | `shandong-wolf/gdscripts/combat_judge.gd`（Node 判定协调器：窗口登记 → 弹反/拼刀/格挡/受击裁决（CLASH_PRIORITY 常量）→ 实体接口调用 → 五结果事件 + 防重入 + 敌人窗口 override fallback 链（#682：windup/hp_damage 读实体瞬时字段）+ 弹反窗口/回报差异化（#720：按 windup 注入 parry_window_seconds/parry_stance_damage，fallback 常量）） | ✅ | GDD |
| Shandong Wolf — AttackWindow 窗口契约 | `shandong-wolf/gdscripts/combat_attack_window.gd`（RefCounted 纯数据：hit_frame/is_active 闭区间/is_expired + parry_window_seconds/parry_stance_damage（#720：-1 fallback 常量，按招式差异化），#581 敌AI 与玩家共用） | ✅ | GDD |
| Shandong Wolf — CombatJudge 测试套件 | `shandong-wolf/tests/test_combat_judge.gd`（25 用例：弹反边界 5/拼刀+冲突矩阵 4/格挡受击 5/崩解+事件契约 5/边界失败路径 6，免树 headless tick_frame，95 断言；#718 连续受击崩解用例 _test_25） | ✅ | GDD |
...
| Shandong Wolf — EnemyAI 行为层 | `shandong-wolf/gdscripts/enemy_ai.gd`（CharacterBody2D 行为 FSM 持有：patrol/chase/attack/retreat + 120° 视线 6m 几何感知（无 raycast/Area2D）+ 决策门控（实体非 idle/move 不决策）+ 弹反抑制窗 0.5s + decide/_physics_process 分离 + 运行时驱动链（#703/#710：_physics_process 先 decide 后 _apply_movement，decide 纯决策回归 + 位移每帧恰一次 + 击退无条件可达）+ @export elite_mode 精英档位 + hit_landed 订阅受击击退（#682）+ 防御触发（_on_player_state_changed 掷骰 ENEMY_BLOCK_CHANCE → GuardState，#720）） | ✅ | GDD |
| Shandong Wolf — EnemyAI 行为状态对象 | `shandong-wolf/gdscripts/enemy_ai_states.gd`（Patrol/Chase/Attack/Retreat 4 态，combat_states.gd 范式派生；AttackState 出招三选一（elite_mode 门控 + 蓄力重斩 override 注入，#682）+ GuardState 防御行为态 + thrust 中前摇注入（#720：ENEMY_THRUST_WINDUP override）；绝不直接改 entity.state_name——AI 行为态与战斗 11 态双命名空间） | ✅ | GDD |
| Shandong Wolf — EnemyAI 测试套件 | `shandong-wolf/tests/test_enemy_ai.gd`（48 用例 Scenario A-H：主路径/感知边界/弹反硬直与崩解/5% 回避/常量驱动/边界失败/回归/运行时驱动链（TC703-1~4：运行时索敌追击/攻击态出招/击退正交门控/位移单次执行，_tick 走 _physics_process 运行时路径，#703），run_tests.gd 第 10 套件） | ✅ | GDD |
...
| Shandong Wolf — 反馈 E2E 截图 rig | `shandong-wolf/gdscripts/e2e_feedback_capture.gd` + `scenes/e2e_feedback_capture.tscn`（CaptureRig 模式：current_state 轮询 + inject_feedback + digit 键 4/6/7/2 + 冻结效果帧；#675 Backdrop z_index -2 恢复火花可见；#661 组级 autoplay 冻结接线 freeze_effects=true + auto_cycle_frames=200）+ `e2e_shots.json` feedback group（3 shots） | ✅ | GDD |
...
| Shandong Wolf — MainBattle 组装编排 | `shandong-wolf/gdscripts/main_battle.gd`（Node2D BattleAssembler：13 步程序化装配+bind（玩家/敌人/Judge/HUD/Reaction/Revive/Execution）+ 游戏状态机 IDLE→COMBAT→KILL→AFTERGLOW→FAIL + 失败字幕/教学提示/击杀余韵编排 + 低血氛围单点接线 + 敌人装配 life_1_max=C.ENEMY_HP_MAX + elite_mode=true（#682 HP 慢线接通）+ #694 facing→scale.x 视觉翻转接线 _sync_visual_facing + set_move_speed 步频接线，零改动 17 组件） | ✅ | GDD |
| Shandong Wolf — PauseMenu 暂停菜单 | `shandong-wolf/gdscripts/pause_menu.gd`（CanvasLayer layer=2 ALWAYS 暂停唯一持有者：自检 game_pause 边沿 + toggle_pause 唯一入口（幂等 + FAIL 守卫）+ 树级暂停 get_tree().paused + 运行时从 InputMap 生成操作手册，零 tscn 零贴图，#719） | ✅ | GDD |
...
| E2E Review Harness | `scripts/run-e2e-review.sh` + `scripts/e2e/resolve_plan.py` + `scripts/e2e/analyze_bmp.py` + `framework/templates/e2e_capture.gd`（#586 管线修复：组级场景键提升/数字 state 直比/results.json 元数据透传/分辨率从 project.godot 读取/--size 尺寸断言；#661 增补：组级 autoplay 提升 + scene_groups 多场景拆分——每场景一个 godot 进程，产物合并） | ✅ | GDD |
...
| 586 | Shandong Wolf 端到端验证（E2E 剧本 + 用户裁决: e2e_script 组 6 帧情感弧剧本（scene_description/trigger/composition 文字契约 AC1）+ rig 4→7 态（MOVE 位移断言/CLASH judge 主路径/EXECUTE 特写, dwell 定长 settle 不跨态）+ 管线四修复（resolve_plan 组级键提升/e2e_capture 数字 state 直比+元数据透传/run-e2e-review 分辨率读取/analyze_bmp --size）+ headless 三档语义 + docs/TEST/586 报告模板, AC1-AC3 管线全过, AC4 用户 6 帧 1-5 星裁决待定稿, #673) | ✅ 已合并 | DESIGN / GDD |
| 681 | Shandong Wolf 攻击动画查询委托补漏 (StickFigureController 补 get_animation_position()/is_animation_playing() 两个 null-guard 薄委托（委托 _anim.current_animation_position / _anim.is_playing()，默认 0.0/false），一处实现同时修复 AnimStateAttack 主路径与 E2E capture 路径的缺失方法 SCRIPT ERROR，attack 三段 phase 判定恢复, #692) | ✅ 已合并 | DESIGN / GDD |
| 683 | Shandong Wolf 火柴人结构完整化 (NeckPivot 颈段 + 头径 16→9.5 比例校准 + 可选 HeadOutline 冷白轮廓（taste 实验 1）+ 腿两段化 LegKPivot 膝 pivot + anim_move 重排 24 帧步态循环（FRAME_ANIM_MOVE_CYCLE=24，contact/pass 关键姿态 + 膝 track + 摆臂反向同频）+ REST_POSE 公共基准衔接规约（R1 动作型首/尾帧 = REST_POSE（≤5°）/ R2 hold 型尾部归位段 FRAME_ANIM_*_EXIT=3，姿态差 ≤ POSE_DELTA_MAX_DEG=15° 枚举断言）+ set_move_speed 步频速度同步（speed_scale 0.3–1.2）+ facing 视觉翻转接线（main_battle._sync_visual_facing → StickFigure.scale.x，物理层零影响红线），AC1-AC3 全过 + F1/F2 适配 + AC 系列单测 + e2e_shots.json move 拆 02a/02b + 03_head_readability，E2E 截图裁决 taste 实验 1/2/3, #694) | ✅ 已合并 | DESIGN / GDD |
| 682 | Shandong Wolf 精英 Boss AI (EnemyAI.elite_mode 参数档位 + 蓄力重斩三选一（CombatEntity 瞬时 override ×2 + judge fallback 链，windup 20 帧可弹反）+ 受击击退位移（hit_landed 订阅 + 衰减 + clamp）+ 敌人架势脱战恢复（2.5s 延迟 + 20/s 节奏阀）+ EnemyHealthBar 敌人血条（_HudBar 复用 + set_fill_color，与 EnemyStanceBar 双条组合）+ HP 慢线装配消费 ENEMY_HP_MAX=80（7 刀慢线 / 4 弹反快线处决双轨击杀），AC1-AC8 全过, #695) | ✅ 已合并 | DESIGN / GDD |
| 684 | Shandong Wolf 敌人 Boss 血条 UI (只狼式顶部血条+架势条组合: EnemyNameLabel 敌人名字 Label（锚点 0.5 + 无底框 + 超长省略）+ _HudBar.set_break_flash 崩解闪白状态机（Tween 0.18s 淡出，无竞态）+ set_boss_mode Boss/杂兵分档三态显隐 + main_battle 装配 Boss 档（「雪夜刀客」占位文案）+ E2E 3 新态（BOSS_BAR/STANCE_BREAK_FLASH/MINION_MODE）；5 新常量 # DRAFT 归 #584；纯消费端零新信号源零贴图, AC1-AC4 全过, #701) | ✅ 已合并 | DESIGN / GDD |
| 703 | Shandong Wolf 敌人 AI 运行时驱动链 (EnemyAI._physics_process 先 decide 后 _apply_movement —— decide 纯决策回归（移除内部位移调用）+ 位移每帧恰一次 + 击退与决策门控正交（stagger 硬直中击退照常执行，AC3）+ 测试 _tick 迁移运行时路径（TC703-1~4，36→48 用例），修复「敌人站桩」（decide 从未被调用，FSM 从未推进）——#695 E2E 只验视觉漏网根因, AC1-AC5 全过, #710) | ✅ 已合并 | DESIGN / GDD |
| 704 | Shandong Wolf 腿 Line2D 方向修正 (bug: 腿画反/与躯干重叠——_make_limb 统一 -Y 工厂对腿语义错配，大腿自髋向 -Y 与躯干 [0,-44] 整段重叠；修复：腿/膝 Line2D 反向 +Y（_reverse_limb 辅助，points[1] 翻向 +Y 长度不变）+ 膝 pivot 移至 (0,+UPPER) + controller 摆姿注释 ±Y 勘误 + DESIGN #683 §2.2/§2.3 勘误；动画 11 clip track 数值零改动红线，REST_POSE「0=直立」勘误为「0=自然下垂」；Scenario D 方向断言 6 条，AC1-AC3 全过, #709) | ✅ 已合并 | DESIGN / GDD |
| 713 | Shandong Wolf 敌人出生点外移 (battle_stage.tscn EnemySpawnA (700,560)→(1000,560)：出生间距 360px > HITBOX_RANGE=80 消除开局空气命中；修布局不修判定——constants/combat_judge/enemy_ai/main_battle 冻结零改动（HITBOX_RANGE=80=ENEMY_ATTACK_RANGE=80 双向对称契约保留）；B4 出生间距断言（> HITBOX_RANGE+50 硬门槛 ≥130px）防回归；AC1-AC3 全过, #716) | ✅ 已合并 | DESIGN / GDD |
| 661 | Shandong Wolf E2E 截图链路修复 (resolve_plan 组级 autoplay 提升（_GROUP_PROMOTED += autoplay，首激活组 wins）+ 新输出 scene_groups（按 main_scene 归类 shots，单场景逐字节不变）+ run-e2e-review.sh 多场景拆分（>1 场景按组多次 capture，产物合并）+ e2e_shots.json feedback 组组级 autoplay 冻结接线（freeze_effects=true + auto_cycle_frames=200 > 最大 settle 120），fb 三档截图必落特效帧；战斗代码零改动红线, #678) | ✅ 已合并 | DESIGN / GDD |+ execute_kill 杀敌通道（绕过 take_damage no-op 红线 + _is_final_dead 停摆）+ set_invincible 玩家无敌 + recover_from_break 起身疲惫（50% 架势 + 5s ×1.2 增伤）+ ExecutionFade 墙钟淡出 + 「处决演出」7 常量 # DRAFT 归 #584 + E2E execution 组 2 shot + 六组测试套件, AC1-AC5 全过, 慢动作/淡出/范围等 DRAFT 值待用户 AC5 裁决, #660) | ✅ 已合并 | DESIGN / GDD |
| 718 | Shandong Wolf stagger 态架势崩解丢失修复 (bug: illegal transition stagger -> stance_break——状态表 stagger 行补 stance_break + parry_success 行补 stance_break（防御加固 1）+ take_stance_damage 补 execute no-op 守卫（防御加固 2，与 take_damage 对齐）；修拓扑不修调用时序，判定层/状态对象/处决编排零改动, AC1-AC3 全过, #727) | ✅ 已合并 | DESIGN / GDD |
| 719 | Shandong Wolf ESC 暂停菜单与操作手册 (PauseMenu 树级暂停（get_tree().paused）+ 自检 game_pause 边沿（A2：InputController 零改动）+ 操作手册运行时从 InputMap 生成（AC3 机器保证）+ FAIL 终态守卫（幂等不弹菜单）+ 恢复路径唯一入口 toggle_pause(false) 缓冲清理；菜单标题/手册措辞/遮罩色值 # DRAFT 归 taste 通道, AC1-AC4 全过, #728) | ✅ 已合并 | DESIGN / GDD |
| 720 | Shandong Wolf Boss级 AI 战斗交互升级 (敌人蓄力期霸体（windup 期不打断）+ 玩家攻击自动面向（攻击瞬间转向）+ 停距/击退缩短（ENEMY_ATTACK_RANGE 65 / ENEMY_KNOCKBACK_PX 22）+ 防御行为（GuardState → block_held）+ 差异化前摇（combo 12 / thrust 16 / charge 24）+ 弹反窗口/回报差异化（0.2s/35、0.25s/45、0.3s/55）；数值 # DRAFT 归 #584, AC1-AC6 全过, #730) | ✅ 已合并 | DESIGN / GDD |
...
#681 攻击 SCRIPT ERROR（StickFigureController 缺失 get_animation_position()/is_animation_playing() callee——#574 实现期调用方契约先写死、callee 漏实现，每次攻击 AnimStateAttack.update 每帧报 Invalid call. Nonexistent function）已由 #692 修复合并（+2 薄委托方法，null-guard 语义 0.0/false，AnimStateAttack 主路径与 E2E capture 兄弟缺陷一并修复，GDD 07 §10 落盘）。当前无新增已知问题。
#684 敌人 Boss 血条 UI 数值全量 # DRAFT（constants.gd「Boss 血条 UI」分区 5 常量：HUD_ENEMY_NAME_WIDTH/FONT_SIZE/TOP + HUD_STANCE_BREAK_FLASH_SECONDS/COLOR + 敌人名字文案「雪夜刀客」占位）定稿归 #584 调参面板/用户裁决；碎裂提示（PRD 4.2-B）与 HP 百分比数字文本（4.4-B）留 taste 候选未实现；GDD 10 §10 落盘。
#682 精英 Boss AI 数值全量 # DRAFT（constants.gd「精英 AI」分区 8 项 + ENEMY_HP_MAX 40→80 上调 + HUD_ENEMY_HP_GAP：蓄力前摇/伤害/概率/击退位移与衰减/脱战恢复延迟与速率 + EnemyHealthBar 视觉色相）定稿归 #584 调参面板/用户裁决；速度/冷却精英差异候选记入 #584 候选池（MVP 维持 #581 默认，范围收敛）；GDD 19 章落盘。

#683 火柴人结构完整化的 taste 决策点（constants.gd「火柴人结构」分区 # DRAFT 候补值：头轮廓去留实验 1 / 步态帧数 24/28/32 实验 2 / 速度同步策略实验 3）待用户 E2E 截图（02a_move_contact/02b_move_pass/03_head_readability）裁决 → docs/TASTE.md；MOVE_SWING_LEG_DEG/MOVE_SWING_ARM_DEG 实现期偏离（25→12，AC3 衔接阈值 POSE_DELTA_MAX_DEG=15° 冲突）已登记 02-CONSTANTS.md。
#661 E2E 截图链路缺陷（resolve_plan _GROUP_PROMOTED 白名单缺 main_scene/autoplay → 组级场景 override 被静默丢弃 → fb 组 shots 跑错场景 + state 枚举错位（EXECUTE=3 在 12 态 stick rig 上是 ATTACK_BURST），fb_execute 截图必为「普通对峙」）已由 #678 修复合并（组级 autoplay 提升 + scene_groups 多场景拆分 + fb 冻结帧接线：freeze_effects=true + auto_cycle_frames=200，#673 先行落地 main_scene/state 提升，GDD 18 §9 落盘）。当前无新增已知问题。
#703 敌人 AI 不行动（decide() 从未被调用——生产驱动链未接线，FSM 从未推进，敌人站桩；#695 E2E 只验视觉漏网根因）已由 #710 修复合并（运行时驱动链：_physics_process 先 decide 后 _apply_movement，decide 纯决策回归 + 位移每帧恰一次 + 击退无条件可达，GDD 13 §13 落盘）。当前无新增已知问题。

#704 腿画反/与躯干重叠（#683 火柴人结构完整化后的视觉回归：_make_limb 统一 -Y 工厂对腿语义错配，大腿自髋向 -Y 与躯干整段重叠，膝 pivot 挂 -Y 侧）已由 #709 修复合并（腿/膝 Line2D 反向 +Y + _reverse_limb 辅助 + 膝 pivot 移至 (0,+UPPER) + controller 摆姿注释 ±Y 勘误 + DESIGN #683 §2.2/§2.3 勘误；动画 11 clip track 数值零改动，D1 方向断言 6 条回归保障，AC1-AC3 全过）。当前无新增已知问题。
#713 敌人出生点距玩家仅 60px（< HITBOX_RANGE=80px）出生即在攻击范围内、开局挥剑空气命中（出生布局与攻击判定范围错配，非判定层 bug）已由 #716 修复合并（battle_stage.tscn EnemySpawnA 700→1000 外移，间距 360px > 80；修布局不修判定——constants/combat_judge/enemy_ai/main_battle 零改动冻结，B4 间距断言 ≥130px 防回归；GDD 14 §9 落盘）。当前无新增已知问题。

#719 暂停菜单/操作手册的 taste 值（constants.gd「PAUSE_*」分区：PAUSE_TITLE_CANDIDATES 三候选（implement 取「暂停」首项）/PAUSE_EXECUTE_LINE_CANDIDATES 两候选（implement 取「处决：架势崩解后按攻击键（J）」首项）/PAUSE_DIM_COLOR 遮罩色值/字号）为 # DRAFT 候选值，定稿归 human-review 通道 → docs/TASTE.md；GDD 20 章落盘。
#720 Boss级 AI 战斗交互升级数值全量 # DRAFT（constants.gd「战斗交互/Boss」相关：ENEMY_ATTACK_RANGE 65 / ENEMY_KNOCKBACK_PX 22 / POSTURE_HIT_COST 18 / ENEMY_CHARGE_WINDUP 24 / ENEMY_THRUST_WINDUP 16 / ENEMY_BLOCK_CHANCE 0.4 / PARRY_WINDOW_CHARGE_SECONDS 0.3 / PARRY_WINDOW_THRUST_SECONDS 0.25 / PARRY_STANCE_DAMAGE 35 / PARRY_STANCE_DAMAGE_CHARGE 55 / PARRY_STANCE_DAMAGE_THRUST 45 + 候选集）定稿归 #584 调参面板/用户裁决；差异化前摇的视觉信号（举刀姿态/刀光节奏）为 taste 骨架，表现细节归 implement 视觉层 + #584 用户裁决；GDD 21 章落盘。
