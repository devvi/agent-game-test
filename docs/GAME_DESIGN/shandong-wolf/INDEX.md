...
| [19-ELITE-BOSS-AI](19-ELITE-BOSS-AI.md) | 精英 Boss AI — EnemyAI.elite_mode 参数档位 + 蓄力重斩（AttackState 三选一 + CombatEntity 瞬时 override ×2 + judge fallback 链）+ 受击击退（hit_landed 订阅位移层）+ 敌人架势脱战恢复（_process 轮询节奏阀）+ EnemyHealthBar 敌人血条（_HudBar 复用 + set_fill_color 覆写，与 EnemyStanceBar 双条组合）+ HP 慢线装配消费 ENEMY_HP_MAX=80 + 精英 AI 分区 9 常量 # DRAFT 归 #584（#682/#695） |
