...
| `mini-pong/tests/run_tests.gd` | ✅ 注册 18 个套件（含 test_upgrade_pool / test_wave_cycle / auto_play） | ❌ 无升级 UI 测试 |
...
| 消费者 | 依赖内容 | 协调点 |
|--------|---------|--------|
| #390 波次转场（研究 PRD 已合入 #429，实现未开工） | 同挂 `wave_settled` | 升级 UI 打开期间转场应让位；两者均监听 `wave_settled`，互不干扰（各自独立信号处理） |
