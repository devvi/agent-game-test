...
- Vault 挂载于 `/Volumes/Obsidian`（WebDAV，`OBSIDIAN_VAULT_PATH`），vault 根 = `Knowledge Ocean/`（wiki/ 74 篇笔记）；本节为 research 复核检索日志（2026-08-13）
- **主命中（已引用）**：《体验引擎》模式 **"可预测的奖励变得无聊"**（`wiki/体验引擎-patterns.md:79-80`，"在不可预测的时间间隔传递奖励"）—— 支撑双得分制的分值差异设计：拆砖 1 分（高频可预测）+ 穿墙 3 分（稀缺高价值，穿墙动作本身不可预测）形成奖励节奏差异，与 PLAN §2.2 注释"穿墙是稀缺高价值动作"一致
- **次要命中（理论背景，非计分专项）**：
  - `wiki/游戏设计理念.md:50`（rogue-like）—— roguelike 玩法演变背景，无计分规则内容
  - `wiki/矮人要塞—程序生成文本的文学边界.md:83`（"胜利条件"）—— "没有胜利条件，每一次失败都是独一无二的终点"，与 PLAN §2.4 失败即叙事呼应，不涉及计分数值
  - `wiki/体验引擎——游戏设计全景探秘.md:128`（"终局"）—— 心理学终局（扑克式心理战），非本 Issue 的 21 分终局语义
- **检索日志（wiki/ 全目录 grep）**：关键词 得分/计分/砖墙/穿墙/触球/双得分/21分/砖块/打砖/breakout/Pong/乒乓/波次/分数 → **0 命中**；rogue/终局/胜利条件/奖励 → 上述 4 篇；`raw/` 目录因 NAS WebDAV 挂载超时未完成（wiki 为优先层，结论不受影响）
- **结论**：知识库无"双得分制/21 分制"专项笔记；玩法决策以 `docs/PLAN-rogue-pong.md` §2.2/§2.4 为唯一权威（该文档本身引用 Obsidian 理论），research 以 PLAN 为准
