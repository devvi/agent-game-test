...
| BgPulse 呼吸相位 → R_bg 主色变化 | 三区同帧比较（AC3 是"区际"距离非绝对色）；呼吸 tint 蓝系与板青/砖橙距离仍 ≥60（PRD §5.2）。**#485 已采纳动态 bg 采样（`bg_sample`）**：bg_ref 帧内实测，排除逻辑对任意相位鲁棒（见 `docs/DESIGN/485-bgpulse-bg-bucket-leak.md` §3.1） |
